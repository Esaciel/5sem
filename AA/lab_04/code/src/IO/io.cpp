...
shared_ptr<vertex> find_or_create_vertex(unordered_map<string, shared_ptr<vertex>> &vmap, const string &label,
                                         uint &index)
{
...
void parse_graphviz(graph &g, istream &in)
{
    ...
    string line;
    regex edge_regex(R"(^\s*([\w.-]+)\s*->\s*([\w.-]+)\s*\[\s*label\s*=\s*(\d+)\s*\]\s*;?\s*$)");
    smatch match;
    uint vertex_index = 0;
...
void print_graphviz(const graph &g, ostream &out)
{
    ...
    for (const auto &e : g.edges)
    {
        if (e->from && e->to)
        {
            out << "    " << e->from->label << " -> " << e->to->label << " [label=<" << e->value << ">];\n";
        }
    }
    out << "}\n";
}
...
void print_pairs(const vector<Pair> &vec, const graph &g, ostream &out)
{
    ...
    for (const auto &p : vec)
    {
        if (f)
            out << ",";
        out << "[" << g.vertices.find(p.first)->second->label << "," << g.vertices.find(p.second)->second->label << "]";
        f = true;
    }
    out << "]" << endl;
}
...
#include <limits>
void print_adjMatrix(const vector<vector<uint>> &adj, ostream &out)
{
    for (const auto &row : adj)
    {
        for (const auto &val : row)
        {
            if (val != numeric_limits<uint>::max())
                out << val << '\t';
            else
                out << "x" << '\t';
        }
