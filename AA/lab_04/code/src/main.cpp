...
int main()
{
    cout << "Задание: Дан граф. Найти все пары вершин на расстоянии (сумма меток дуг на пути) не большем, чем заданная "
    ...
    double max_gap;
    cin >> max_gap;
    cout << "Введите число потоков (0 - выполнится непараллельная реализация)" << endl;
    uint threads_num;
    cin >> threads_num;
    vector<Pair> res;
    if (!threads_num)
    ...
    cout << "Множество пар [A,B], у которых расстояние от A до B меньше " << max_gap << ":" << endl;
    print_pairs(res, g);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    {
        cout << measure(adj, max_gap, threads_num, 100) << endl;
        return 0;
    }
}
#else
